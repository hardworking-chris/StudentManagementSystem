  ...
  <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm" size="medium">
    ...
    <el-form-item label="生年月日" required>
      <el-form-item prop="birthday" style="width: 20%;">
        <el-date-picker  type="date" placeholder="日付を選んでください" value-format = "yyyy-MM-dd" v-model="ruleForm.birthday" style="width: 100%;"></el-date-picker>
      </el-form-item>
    </el-form-item>
    ...
    <el-form-item label="入塾時間" required>
      <el-form-item prop="entryDate" style="width: 20%;">
        <el-date-picker  type="date" placeholder="日付を選んでください" value-format = "yyyy-MM-dd" v-model="ruleForm.entryDate" style="width: 100%;"></el-date-picker>
      </el-form-item>
    </el-form-item>
    <el-form-item label="卒業時間" required>
      <el-form-item prop="graduateDate" style="width: 20%;">
        <el-date-picker  type="date" placeholder="日付を選んでください" value-format = "yyyy-MM-dd" v-model="ruleForm.graduateDate" style="width: 100%;"></el-date-picker>
      </el-form-item>
    </el-form-item>
    ...
    <el-form-item>
      <el-button type="primary" @click="submitForm('ruleForm')">学生編集</el-button>
...
export default {
  data () {
    return {
      ...
      rules: {
        ...
        birthday: [
          {type: 'string', required: true, message: '日付を選んでください', trigger: 'change'}
        ],
        entryDate: [
          {type: 'string', required: true, message: '日付を選んでください', trigger: 'change'}
        ],
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.put('http://localhost:8080/update', this.ruleForm).then(resp => {
            if (resp.data === 'success') {
              this.$alert('編集成功', 'メッセージ', {
  ...
  created () {
    axios.get('http://localhost:8080/findById/' + this.$route.query.id).then(resp => {
      if (resp.data.english != null) {
        let var1 = resp.data.english.trim().split(/\s+/)
        let var2 = []
        let var3 = {}
        for (let i = 0; i < var1.length; i++) {
          ...
          if (i % 2 === 1) {
            var3[var1[i - 1].toString()] = (var1[i].toString())
          }
        }
        resp.data.englishTemp = var2
        resp.data.scores = var3
